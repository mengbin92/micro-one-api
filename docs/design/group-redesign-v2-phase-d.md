...
新增 `095_create_routing_change_outbox.sql`，提供 MySQL、PostgreSQL、SQLite 三个方言，ownership 同时列入 identity 和 channel。在拆分 schema 时分别落在所有者库；共享 schema 由迁移版本记录保证只应用一次。
...
1. 保持 C 阶段的回填与请求快照前提，安装 095；identity/channel 启动就绪检查现在包含 outbox 表。已有 092/093/094 不做破坏性重写。
