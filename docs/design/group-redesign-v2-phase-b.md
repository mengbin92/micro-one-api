...
channel 新增稳定的路由分组 ID、两类资源成员表、模型映射/模型路由的数字组引用，以及按报告摘要记录的回填进度。迁移 `092_create_routing_groups.sql` 有 MySQL、PostgreSQL、SQLite 三个版本，所有权归 channel。
...
| 检查 | 结果 |
|---|---|
| MySQL 8、PostgreSQL 16、SQLite 实际执行 092 | 通过 |
