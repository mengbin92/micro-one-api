...
>
> 用户已确认：**禁止静默降级；未实现/协议不兼容直接报错，保留显式配置的同模型故障切换。**
...
- 未实现/协议不兼容错误类型化并禁止触发换 provider 或隐式协议替换；检查已有 Responses→Chat 等能力降级分支，按已确认边界改为明确报错并更新契约测试。
...
| 4：未实现直接错误、实际 channel、SSE 空闲/总超时分开 | R3/R4/O2；按用户确认保留显式同模型故障切换，禁止静默协议/模型降级；已有空闲实现优先复用 |
