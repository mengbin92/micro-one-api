...
新增三方言迁移：`096_identity_ordered_token_groups`（identity）、`097_billing_user_routing_price_overrides`（billing，heads+history）、`098_channel_routing_relation_overrides`（channel，4 个可空列），均已登记 ownership.yaml。全部纯增量，096/097 新表、098 可空列，旧版本服务安全忽略。

启用顺序：

1. 部署 identity/billing/channel（096/097/098），纯增量无需回填。
...
| 检查 | 结果 |
|---|---|
| 三方言 096/097/098 完整迁移及重复应用（无操作断言） | 通过 |
