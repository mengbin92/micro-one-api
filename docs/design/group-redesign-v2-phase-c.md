...
> 运行状态更新（2026-09-13）：本阶段实现记录保留历史执行范围；当前部署、开关、迁移与能力以[最新只读运行基线](../runbooks/routing-baseline-2026-09-13.md)为准。
...
迁移 `094_add_identity_routing_facts.sql` 分别提供 MySQL、PostgreSQL、SQLite 版本，所有权归 identity：
...
迁移 `093_add_request_snapshots.sql` 的三个方言版本归 billing 所有。reservation 新增 nullable `request_snapshot`、`request_snapshot_hash`、`subscription_accounting_usd`；共享订阅领域通过同一结算事务写 `subscription_window_charges`，记录原订阅/策略/三个窗口及实际 accounting USD。
...
| 服务 | 开关及前提 |
|---|---|
| billing | `BILLING_REQUEST_SNAPSHOT_V2=true`；093 schema；`CHANNEL_GRPC_ENDPOINT` 和既有 `SERVICE_TOKEN` |
| identity | `IDENTITY_ROUTING_V2=true`；094 schema 和用户回填完成；channel 连接；服务端默认组有效 |
| relay | `RELAY_ROUTING_CONTEXT_V2=true`；identity、channel、billing 均理解本阶段契约 |

以下是待上线时执行的顺序，本阶段未执行生产操作：

1. 完成 B 的组回填，并保证所有 channel 写实例持续双写。安装 093/094 additive schema。
