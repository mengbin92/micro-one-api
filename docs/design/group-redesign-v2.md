...
> 日期：2026-09-08
> 状态：分阶段实施中；阶段 A、B 已完成，C—F 未开始。详细状态与验证证据见 §8.2；尚未迁移生产数据库或切换运行行为。
...
| C | 未开始 | 下一阶段：identity 默认组/授权事实、inherit 请求快照；必须覆盖同步、异步两条结算路径 |
| D | 未开始 | 等待 C；固定组开放前验证全链路组一致及撤权 |
| E | 未开始 | 等待 D；仅订阅模式受硬额度闸门约束 |
| F | 后续阶段 | Auto 与高级定价按第 10 节单独验收 |
...
- [可用组与 Auto 解析](../../../new-api/service/group.go)：`GetUserUsableGroups`、`GetRequestAutoGroups`、`GetUserGroupRatio`。
- [Token](../../../new-api/model/token.go)、[运行时鉴权](../../../new-api/middleware/auth.go)、[组间调度](../../../new-api/service/channel_select.go)。
- [倍率配置](../../../new-api/setting/ratio_setting/group_ratio.go)、[abilities](../../../new-api/model/ability.go)。
- [订阅套餐与用户组变更](../../../new-api/model/subscription.go)：UpgradeGroup、DowngradeGroup、PrevUserGroup 及购买/到期处理。

### 同级 sub2api

- [Group 模型](../../../sub2api/backend/internal/service/group.go)、[account_groups](../../../sub2api/backend/ent/schema/account_group.go)。
- [用户资格](../../../sub2api/backend/internal/service/user.go)、[用户授权关系](../../../sub2api/backend/ent/schema/user_allowed_group.go)。
- [APIKey 模型](../../../sub2api/backend/internal/service/api_key.go)、[Key 创建和可绑定组](../../../sub2api/backend/internal/service/api_key_service.go)、[运行时组/订阅检查](../../../sub2api/backend/internal/server/middleware/api_key_auth.go)。
- [用户专属组倍率](../../../sub2api/backend/internal/service/user_group_rate_resolver.go)、[用户订阅关系](../../../sub2api/backend/ent/schema/user_subscription.go)。

同级源码链接用于本地对照；独立检出本项目时需要同级工作副本。涉及参考项目能力的结论固定于文首 commit，本提案未修改这两个项目。
