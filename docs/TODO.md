...
> 最后更新：2026-09-22
>
> **当前执行入口**：[下一阶段计划：可靠性补缺、运行可见性与文章技术债收口](./design/next-stage-plan-2026-09-22.md)。基于 `develop@77e4db45`（v0.31.0），汇总桌面 22 篇原文、四组补充建议及未结束验收；优先级、依赖与退出条件以新计划为准。下文保留历史登记，不作为当前待办。
>
> **第一批已完成**：R1 凭证轮换/CAS/周期补写与告警、R2 低流量熔断、R3 明确能力错误/501、R4 流式预算及隔离矩阵均通过本地验收，见[实施记录](./runbooks/first-batch-reliability-2026-09-22.md)。实现与验收记录已纳入本批提交，未部署。下一步为 O1–O4；O5 按测量推进，生产类型盘点为部署前置，外部告警送达仍属 O2。
>
> **已交付基线**：[v0.30 路线图](./design/v0.30-roadmap.md)已归档；[v0.31.0](./releases/release-v0.31.0.md)已发布结算恢复、请求尝试/实际来源追踪和跨来源切换。流式隔离矩阵已补齐（MySQL/SQLite 共 24 场景）；F17 支付沙箱与跨重启/多副本凭证边界仍按新计划跟踪。
>
...
> [下一阶段计划](./design/next-stage-plan-2026-09-22.md) 管理。
>
...
>
> 📣 **v0.17 阶段（2026-08-10，已收尾）**：P3.1 Linux/amd64 三版本基线复测 + P3.2 jsonx
...
**背景**：Anthropic 协议的 `cache_creation_input_tokens`（含
`cache_creation.ephemeral_5m/1h_input_tokens` TTL 细分）在全链路没有承载字段，
