...
>
> **下一步顺序**：R1 凭证轮换与周期补写 → R2 低流量熔断 → R3 明确错误/501/受控同模型切换 → R4 流式超时与隔离验收；之后推进统一观测、告警、冻结额度与账号治理。多副本凭证协调是扩容前置门，其余产品/架构扩展按触发条件启动。
>
> **已交付基线**：[v0.30 路线图](./design/v0.30-roadmap.md)已归档；[v0.31.0](./releases/release-v0.31.0.md)已发布结算恢复、请求尝试/实际来源追踪和跨来源切换。流式隔离矩阵、F17 支付沙箱及持久化持续失败边界承接到新计划，不以发布或服务健康代替验收。
>
