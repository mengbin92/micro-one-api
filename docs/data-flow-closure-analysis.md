...
更新：2026-09-21，前四批修复已部署线上服务，第五批已完成本地实现与验收；保留各轮观测与验收的时间边界。
...
本地验收通过选路事件内容/稳定去重键、用户与根请求隔离回读、跨页数据库排序、健康 unknown/disabled、旧 adminToken 普通用户分流、admin/billing/log/relay 聚焦 Go 回归、`go vet` 和 Web 生产构建。部署不需要新增数据库 migration，但必须同时升级 relay-gateway、log-service、billing-service、admin-api 和挂载的 `web/dist`；随后用一个可控 failover 样本验证 planned/outcome、多个 attempt、fallback 原因和账本引用能在一次管理查询中回读。
...
| 第五批 | F20 选路回读、F21 前端一致性、§6.2 验收记录 | 明确保留/权限、排序范围与导出内容；灰度配置有对应来源级证据，模型健康自动选路另行设计。 | 本地实现与回归完成；待五个服务/前端部署及真实 failover 回读 |
