...
本批尚未宣称线上验收完成：需要在隔离环境执行 Redis consumer 崩溃后的自动重领、notify 多实例/重启及平台业务拒绝、OAuth Store 暂时失败恢复、到期提醒扫描失败恢复和共同保留窗口的真实数据核对；历史日志/渠道补偿禁止直接在生产执行，必须先完成脚本预览并确认不产生扣费或上游重放。
...
本批尚未宣称线上验收完成：需要执行 config/billing migration 并重启服务，隔离验证失败终态统计、配置消费者实际加载版本、丢回调订单的 paid/closed 收敛、查单失败重试，以及站内冲正与外部支付退款状态分别可读。线上不能用一次 GET 或本地 revision 推断所有实例已应用配置。
...
| 第二批 | F11 pending 重领；F18 凭据持久化补偿 | 暂时失败无需新事件/重新登录即可恢复；多实例重复处理有界且安全。 | 代码完成；隔离故障矩阵待验收 |
| 第二批 | F2/F12/F14 提醒、通知 wiring 与状态/回执 | 关闭不写 sent；提醒被受理；业务拒绝、扫描/写状态失败可见；保持现有运营开关意图。 | 代码完成；notify 多实例/平台回执待验收 |
| 第二批 | F3/F4/F5 时间窗、历史基线、日志补偿与去重/删除边界 | 排除保留期影响，仍检出真缺口；补日志不扣费，不重放上游，旧账号事件不灌入当前窗口。 | 代码完成；生产只读核对与人工补偿待验收 |
| 第三批 | F15/F16/F17 模型统计、配置应用版本、支付补查和退款边界 | 终态失败统计可核对；配置可确认已应用；丢回调可收敛，站内冲正与外部退款分开验收。 | 代码完成；本地聚焦测试通过，线上/隔离验收待执行 |
