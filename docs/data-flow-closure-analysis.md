...
更新：2026-09-22，前五批修复已部署线上服务；补充用户授权的生产前置准备，保留各轮观测与验收的时间边界。
...
前三批代码已部署到线上 billing、monitor、config 等服务；线上验收仍待按下方清单执行。需要验证 monitor 回写、对账历史回读、异步任务重启恢复、有限 Key 重试、gRPC 失败终态、配置应用版本及支付查单收敛。会话窗口目前仍由 relay 的本地 Redis 记录，尚未迁移为 billing 可恢复的跨进程派生任务；数据库表只通过新增 SQL migration 创建，业务代码不执行建表。

### 2026-09-20 第二批实施状态

已完成本批代码修复并通过本地全量 Go 包测试（未启动依赖服务的 E2E 除外）：F11 为每个 Redis Stream consumer group 启动定时 `XAUTOCLAIM`，只领取超过空闲阈值的 pending 消息；F18 对 OAuth 刷新后的凭据持久化增加有限重试，失败时保留完整轮换凭据在进程缓存中，避免下一次刷新继续使用旧 refresh token；F2 在告警关闭或 notify endpoint 不可用时不再注入 NoopNotifier，因此不会创建或记录“sent”；F12 将到期提醒从丢弃返回值改为可选持久通知 sink，并按订阅与到期时间抑制重复提醒；F14 增加通知行的条件领取、processing 恢复、租约持有者条件完成、平台业务回执校验和失败重试状态；F3 对账优先使用共同 30 天窗口比较 ledger/log，避免日志保留期制造全生命周期假差异。F4/F5 沿用现有 rollback-first 的 `scripts/reconcile/repair_usage_accounting.sql`，补偿仍必须在固定时间窗、人工核对和显式提交后执行，已删除渠道只保留历史账本身份，不自动覆盖新累计值。

本批已部署线上服务；线上验收仍待完成。需要在隔离环境执行 Redis consumer 崩溃后的自动重领、notify 多实例/重启及平台业务拒绝、OAuth Store 暂时失败恢复、到期提醒扫描失败恢复和共同保留窗口的真实数据核对；历史日志/渠道补偿禁止直接在生产执行，必须先完成脚本预览并确认不产生扣费或上游重放。

### 2026-09-20 第三批实施状态

已完成代码修复并通过本地聚焦验收：F15 将最终失败请求写入模型 `error_count`，模型查询只忽略未注册模型而保留存储错误，日聚合延迟改为按请求数加权平均；F16 将 config-service 配置记录增加持久 `revision`，读回、gRPC/HTTP 写响应均返回版本，配置服务明确只承诺持久化版本，不把未接消费者的 `config.changed` 当作运行实例已应用；F17 增加 billing 有界 pending 支付主动查单任务，查单错误进入可见报告，退款响应明确站内冲正完成且外部原路退款不支持，无法追踪订阅归属的旧订单不再回退到当前 active 订阅。同步了三种数据库方言的 revision migration 和 proto 生成文件。
...
本批本地验收包括：上游失败后释放且第二次尝试共享同一根请求并递增序号；映射/直通模型在预留、提交和日志中一致；重开 SQLite 数据库后仍可按用户和根请求分页读取；同一 attempt 重放不重复扣款；管理查询的认证、参数校验和分页转发；单条与批量日志读回。生产验收还需在隔离环境执行一次可控 failover、强制进程重启后的根请求查询，以及确认未知历史仍未被回填。

工作区审查补充修复：订阅适配器在格式转换前应用当前账号的模型映射，防止实际发送模型与预留来源不一致；滚动升级遇到旧 billing 响应时，relay 保留本次提交的追踪字段用于日志和后续 WS 重试，不回填历史 reservation。对应回归覆盖映射后的实际请求体、脱离请求上下文的日志及连续 WS 重试；测试桩为每次预留返回独立 ID。

### 2026-09-21 第五批实施状态：F20/F21/§6.2

本批已完成代码和本地回归。F20 复用 log-service 已有持久日志与 30 天默认保留策略，将选路计划和执行终态按 `user_id + root_request_id + phase` 幂等保存；记录候选来源类别、最终来源、sticky、fallback/选路原因、结果和耗时，不保存令牌、请求正文或上游响应。relay 的 HTTP、订阅适配器、orchestrator 与 gRPC 路径共用稳定根请求；审计写失败增加低基数指标并输出结构化错误。管理员 `/api/log/routing-audit` 在认证后合并选路事件与 F19 billing attempts，管理端调用日志页提供同一查询。保留清理沿用 log-service 配置，过期后不承诺业务审计永久可查。
...
上述验证只覆盖请求规划前的候选切换。采样版本中 chat candidate-list 路径的命名空间锁仍阻止请求中途从普通渠道跨到订阅账号；禁用映射后在 Plan 阶段选择另一来源也不会增加 `routing_fallback_total`。真实上游失败、跨来源重试、重启后幂等恢复及其余 F7–F18 故障矩阵仍须隔离验收，不能记为 PASS。

2026-09-22 用户明确要求直接在生产进行下一阶段前置准备，并确认渠道 1 的 `kimi-k3` 与 Kimi 订阅账号 5 的 `k3` 为不同供应商的同一模型。已核对 model 143 的两条既有映射均启用，创建用户 2 专用的短期、限定模型、4096 tokens 有限额度令牌，并回读三条已有根请求的审计。补齐 monitor 的 `CHANNEL_GRPC_ENDPOINT=channel-service:9002` 后，ListChannels 与健康回写 RPC 成功；但上游模型列表返回 HTML，主动探测暂设关闭，本次产生的两次健康降级经限定条件恢复，未伪造成功事件。业务镜像、模型映射及计费范围保持原值，未发生成请求或执行故障矩阵。详见[生产前置准备记录](runbooks/data-flow-production-preflight-2026-09-22.md)与[证据](runbooks/evidence/data-flow-production-preflight-2026-09-22.json)。这补齐生产测试输入和回读条件，不消除请求中途跨来源回退、恢复矩阵及外部计费证据缺口。

2026-09-22 后续本地代码修复：Chat Completions 在 hybrid 开启时逐次分派普通 provider 和订阅 adaptor，订阅候选重新校验并解析凭据；adaptor orchestrator 同样显式允许跨来源候选。预扣、日志与审计采用实际尝试的来源及模型，失败预扣释放，成功只结算一次，提交失败不重放上游。订阅到 Chat 的 SSE 转换保留真实 usage，避免成功回退后退化为估算计费。本地 HTTP 回归覆盖渠道 1 `Kimi-K3` 与账号 5 `k3` 双向、流式/非流式 8 个组合及凭据失败、权限撤销、400、提交失败和 hybrid 关闭边界。修复（`31250763`）已于 2026-09-22 03:22 UTC 部署生产 relay-gateway；故障矩阵、重启恢复与外部计费证据仍待验收，历史生产证据保持原样。

| 阶段 | 范围 | 验收 | 状态 |
| --- | --- | --- | --- |
| 第一批 | F13 monitor 实际依赖地址；F1/F6 SaveRun、部分失败状态、差异编码和 DTO | 主动巡检成功且回写可读；一次对账一条历史，七类往返不丢，失败不伪装成功。 | 已部署线上；待 migration/重启及结果回读验收 |
| 第一批 | F7/F8/F9 结算完成事件、账号/会话/Key 回写与失败恢复；F10 入口一致性 | 同步/异步、HTTP/gRPC、重试与重启后分别记账一次；不能通过过期释放掩盖已服务用量。 | 已部署线上；失败矩阵和会话窗口待验收 |
| 第二批 | F11 pending 重领；F18 凭据持久化补偿 | 暂时失败无需新事件/重新登录即可恢复；多实例重复处理有界且安全。 | 已部署线上；隔离故障矩阵待验收 |
...
| 第六批 | 生产只读复核；F7–F18 失败恢复；F19/F20/F22 隔离 failover、重启与来源归属 | 生产查询结果可回读；失败任务可恢复且幂等；通知/支付/配置状态真实可见；根请求、尝试、实际来源和账本一致。 | 阶段 A 只读已完成；另经授权观察到 model 143 双向 Plan 候选切换、F7 新额度事件及 F19/F20/F22 关联；跨来源回退代码已于 2026-09-22 部署生产；同日本地隔离栈完成 F7–F18 + F19/F20/F22 非流式故障矩阵，除 F17 丢回调收敛子项（需支付宝沙箱）外全部通过，矩阵发现并修复 4 个真实缺陷（已于 2026-09-22 06:54 UTC 部署生产 relay/identity/channel，见 [runbook](runbooks/data-flow-next-stage-acceptance.md)）；流式跨来源回退仅单元回归覆盖。常规生产边界仍按 [下一阶段验收 Runbook](runbooks/data-flow-next-stage-acceptance.md) 执行 |
