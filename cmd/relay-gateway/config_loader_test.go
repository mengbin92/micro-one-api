...
func TestLoadConfigRelayOrchestratorAllowlistEmptyAndConfigured(t *testing.T) {
	...
	configYAML := "relay_orchestrator:\n  enabled: false\n  allowlist_token_hmac_sha256: [${MOA_RELAY_TOKEN_HMAC_SHA256:-}]\n"
	if err := os.WriteFile(configPath, []byte(configYAML), 0o600); err != nil {
		t.Fatal(err)
	}

	t.Run("empty", func(t *testing.T) {
		t.Setenv("MOA_RELAY_TOKEN_HMAC_SHA256", "")
		cfg, err := loadConfig(configPath)
		if err != nil {
			t.Fatalf("loadConfig() error = %v", err)
		}
		if got := cfg.Bootstrap.RelayOrchestrator.GetAllowlistTokenHmacSha256(); len(got) != 0 {
	...
	t.Run("configured", func(t *testing.T) {
		...
		t.Setenv("MOA_RELAY_TOKEN_HMAC_SHA256", configuredDigest)
		cfg, err := loadConfig(configPath)
		if err != nil {
			t.Fatalf("loadConfig() error = %v", err)
		}
		got := cfg.Bootstrap.RelayOrchestrator.GetAllowlistTokenHmacSha256()
