...
> 📣 **最新发布**：[v0.26.4 发布公告](./docs/releases/release-v0.26.4.md)（控制台页面切换延迟修复） · [GitHub Release](https://github.com/mengbin92/micro-one-api/releases/tag/v0.26.4)
...
v0.26.3 是替代未完成发布的 v0.26.2 的 **PATCH Relay 路由、双语界面与发布门禁修复版本**：完整包含 v0.26.2 的模型标识归一化、显式渠道模型映射和中英文界面修复，并同步生成 API 类型、兼容本地化 Playwright 定位器。**无公共 API / proto 变更、无数据库迁移**；v0.26.2 未产出完整 Release 制品，请直接更新 `relay-gateway`、同步 `web/dist` 并使用 v0.26.3。详见 [docs/releases/release-v0.26.3.md](./docs/releases/release-v0.26.3.md)。

### 升级到 v0.26.2

v0.26.2 是 v0.26.1 之后的 **PATCH Relay 路由与 Web 界面修复版本**：统一模型标识大小写和 `[1M]` 扩展上下文处理，恢复显式渠道模型映射优先级，并同步中英文界面文案。**无公共 API / proto 变更、无数据库迁移**；更新 `relay-gateway` 并同步 `web/dist`，无需因本版本执行数据库迁移。详见 [docs/releases/release-v0.26.2.md](./docs/releases/release-v0.26.2.md)。
...
v0.24.0 是 v0.23.3 之后的 **MINOR Web 体验与运营基础版本**：完成用户端与管理端的可访问性设计重构，增加中英文界面、中国用户协议与隐私政策、注册显式同意和可配置运营主体信息。**无数据库迁移、无公共 API / proto 破坏性变更、无新增 relay 运行时行为**；部署 `admin-api` 并同步 `web/dist`，详见 [docs/releases/release-v0.24.0.md](./docs/releases/release-v0.24.0.md)。
...
v0.24.0 是 v0.23.2 之后的 **MINOR Web 体验与运营基础版本**：完成用户端与管理端的可访问性设计重构，增加中英文界面、中国用户协议与隐私政策、注册显式同意和可配置运营主体信息。**无数据库迁移、无公共 API / proto 破坏性变更、无新增 relay 运行时行为**；部署 `admin-api` 并同步 `web/dist`，详见 [docs/releases/release-v0.24.0.md](./docs/releases/release-v0.24.0.md)。
...
v0.23.0 是 v0.22.5 之后的 **MINOR 执行边界与灰度能力版本**：新增默认关闭、token allowlist 保护的 Chat Completions 非流式 executor staging 路径，补齐 transport-neutral 端口、adaptor registry、failover 结算和失败矩阵；修复管理后台成本图表标签重叠。**无数据库迁移、无公共 API / proto 破坏性变更**；升级部署 `relay-gateway` 并同步 `web/dist`，保持 `RELAY_ORCHESTRATOR_ENABLED=false` 即继续使用旧路径。详见 [docs/releases/release-v0.23.0.md](./docs/releases/release-v0.23.0.md)。
...
v0.22.2 是 v0.22.1 的 **PATCH 兼容性修复版本**：适配 Kimi K3 对聊天请求参数和输出 token 字段的限制，移除 Kimi K3 不支持的固定采样参数并将 `max_tokens` 映射为 `max_completion_tokens`。**无 API/proto 破坏性变更、无数据库 schema 迁移、无新增配置项**；仅需重新部署 `relay-gateway`，详见 [docs/releases/release-v0.22.2.md](./docs/releases/release-v0.22.2.md)。
