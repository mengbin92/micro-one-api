    ...
    <div className="grid gap-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="grid gap-2">
          <Label htmlFor="model-id">{t("模型 ID")}</Label>
          <Input
            id="model-id"
            value={draft.modelId}
            disabled={isEdit}
            onChange={(e) => onChange({ modelId: e.target.value })}
            placeholder={t("如 gpt-4o, claude-3-5-sonnet")}
          />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="display-name">{t("显示名称")}</Label>
          <Input
            id="display-name"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="grid gap-2">
          <Label htmlFor="context-window">{t("上下文窗口")}</Label>
          <Input
            id="context-window"
            type="number"
      ...
      <div className="grid gap-2">
        ...
        <Input
          ...
          placeholder={t("如 large-language, image, audio")}
        />
      </div>
      <div className="grid gap-2">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="grid gap-2">
          <Label htmlFor="capabilities">{t("能力标签 (逗号分隔)")}</Label>
          <Input
            id="capabilities"
            value={draft.capabilities}
            onChange={(e) => onChange({ capabilities: e.target.value })}
            placeholder={t("如 vision, function_calling, streaming")}
          />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="tags">{t("自定义标签 (逗号分隔)")}</Label>
          <Input
            id="tags"
            value={draft.tags}
            onChange={(e) => onChange({ tags: e.target.value })}
            placeholder={t("如 large-context, fast")}
          />
        </div>
      </div>
      <div className="grid gap-2">
        <Label htmlFor="metadata">{t("元数据 (JSON)")}</Label>
        <Input
          id="metadata"
          value={draft.metadata}
          onChange={(e) => onChange({ metadata: e.target.value })}
          placeholder={t("如 JSON 字符串")}
