...
const healthMeta = {
  ...
  degraded: { label: '性能降级', icon: AlertTriangle, className: 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300' },
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="space-y-4 p-5">
      <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
        ...
        <select aria-label={t('来源筛选')} className="h-10 rounded-md border bg-background px-3 text-sm" value={sourceKind} onChange={(event) => { setSourceKind(event.target.value); setPage(1); }}><option value="">{t('全部来源')}</option><option value="channel">{t('API 渠道')}</option><option value="subscription">{t('订阅账号')}</option></select>
        <select aria-label={t('状态筛选')} className="h-10 rounded-md border bg-background px-3 text-sm" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }}><option value="">{t('全部状态')}</option><option value="healthy">{t('健康')}</option><option value="degraded">{t('性能降级')}</option><option value="unavailable">{t('不可用')}</option></select>
      </div>
      {query.isLoading ? <TableSkeleton columns={[t('状态'), t('模型'), t('上游来源'), t('成功率'), t('平均延迟'), t('连续失败'), t('最后观测'), t('最近错误')]} /> : query.isError ? <div role="alert" className="flex min-h-40 flex-col items-center justify-center gap-3 rounded-2xl border border-destructive/40 bg-destructive/5 px-6 py-10 text-center"><AlertCircle className="size-8 text-destructive" /><p className="text-sm font-semibold">{t('模型健康数据加载失败')}</p><Button variant="outline" size="sm" onClick={() => query.refetch()}>{t('重试')}</Button></div> : states.length === 0 ? <EmptyState title={t('暂无模型健康数据')} description={t('模型完成真实请求后，健康状态会自动出现在这里。')} /> : <div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>{t('状态')}</TableHead><TableHead>{t('模型')}</TableHead><TableHead>{t('上游来源')}</TableHead><TableHead>{t('成功率')}</TableHead><TableHead>{t('平均延迟')}</TableHead><TableHead>{t('连续失败')}</TableHead><TableHead>{t('最后观测')}</TableHead><TableHead>{t('最近错误')}</TableHead></TableRow></TableHeader><TableBody>{states.map((state) => <TableRow key={state.id}><TableCell><HealthBadge status={state.status} /></TableCell><TableCell><div className="font-mono text-sm">{state.model_id}</div>{state.upstream_model_id !== state.model_id && <div className="text-xs text-muted-foreground">→ {state.upstream_model_id}</div>}</TableCell><TableCell>{t(state.source_kind === 'channel' ? 'API 渠道' : '订阅账号')} #{state.source_id}</TableCell><TableCell>{successRate(state)} <span className="text-xs text-muted-foreground">({state.success_count}/{state.request_count})</span></TableCell><TableCell>{numberValue(state.avg_latency_ms).toLocaleString()} ms</TableCell><TableCell>{state.consecutive_failures}</TableCell><TableCell>{formatUnix(state.last_checked_at)}</TableCell><TableCell className="max-w-80 truncate" title={state.last_error}>{state.last_error || '—'}</TableCell></TableRow>)}</TableBody></Table></div>}
