...
export function RedeemPage() {
  ...
  const redeemMutation = useMutation({
    mutationFn: async () => {
      ...
      return unwrapApiData<number>(res.data, t("兑换失败"));
    },
    onSuccess: (amount) => {
      ...
      <section className="overflow-hidden rounded-lg border border-violet-100 bg-white shadow-sm dark:border-white/10 dark:bg-card">
        <div className="grid gap-0 md:grid-cols-[1fr_0.8fr]">
          <div className="p-6 sm:p-8">
            ...
            <h2 className="text-3xl font-black tracking-normal text-slate-950 dark:text-white">{t("兑换码充值")}</h2>
            <p className="mt-2 max-w-xl text-sm font-medium text-slate-500 dark:text-slate-400">{t("输入管理员发放的兑换码，成功后金额会立即入账。")}</p>

            <div className="mt-8 space-y-3">
              <Label htmlFor="redeem-code">{t("兑换码")}</Label>
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {redeemMutation.isPending ? <Loader2 className="size-5 animate-spin" /> : <Ticket className="size-5" />}{t("立即兑换")}</Button>
              </div>
            </div>

            {redeemedAmount !== null && (
              <div className="mt-5 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">{t("兑换成功，已到账")}{formatAmount(redeemedAmount)}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col justify-center bg-violet-600 p-6 text-white sm:p-8">
            <div className="flex items-center gap-3 text-sm font-bold text-violet-100">
              <WalletCards className="size-5" />{t("当前余额")}</div>
            <div className="mt-4 text-5xl font-black">{formatAmount(dashboard?.balance)}</div>
            <div className="mt-3 text-sm font-semibold text-violet-100">{t("兑换后可在使用记录和订单中查看账务流水。")}</div>
          </div>
        </div>
      </section>
    </div>
