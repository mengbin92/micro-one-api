...
function statusLabel(status: PageStatus) {
  switch (status) {
    case 'loading_models': return '正在加载模型';
    case 'submitting': return '正在连接';
    case 'streaming': return '生成中';
    case 'completed': return '已完成';
    case 'stopped': return '已停止';
    case 'failed': return '调用失败';
    case 'ready': return '已验证';
    default: return '等待 API Key';
  }
}
...
function errorTitle(kind: PlaygroundErrorKind) {
  switch (kind) {
    ...
    case 'forbidden_model': return '模型无权限';
    case 'insufficient_quota': return '额度不足';
    case 'rate_limited': return '请求过于频繁';
    case 'upstream_unavailable': return '上游暂不可用';
    case 'invalid_request': return '请求参数有误';
    case 'cors_or_network': return '无法连接 Relay';
    case 'protocol_error': return '响应格式异常';
    case 'aborted': return '请求已停止';
    default: return '调用失败';
  }
}
...
export function PlaygroundPage() {
  ...
  useEffect(() => {
    ...
    resolveRelayBaseUrl()
      ...
      .catch((reason: unknown) => {
        if (!cancelled) setAddressError(reason instanceof Error ? reason.message : 'Relay 地址无效');
      });
  ...
  async function verifyKey(secret = apiKey) {
    ...
    if (!normalized) {
      setModelError(new Error('请先输入 API Key') as RelayPlaygroundError);
      setStatus('needs_key');
      return;
    }
    if (!address) {
      setModelError(new Error(addressError || 'Relay 地址尚未准备好') as RelayPlaygroundError);
      return;
    }
    ...
    try {
      ...
      if (result.length === 0) setModelError(new Error('当前 API Key 没有可用模型') as RelayPlaygroundError);
    } catch (reason) {
      if (modelRequestSequence.current !== modelSequence || modelRequest.current?.controller !== modelController) return;
      const relayError = isRelayError(reason)
        ? reason
        : new Error(reason instanceof Error ? reason.message : '模型加载失败') as RelayPlaygroundError;
  ...
  async function sendMessage() {
    ...
    if (
      ...
      setError(new RelayPlaygroundError('Temperature 或 Max Tokens 超出允许范围', { kind: 'invalid_request' }));
      setStatus('failed');
      return;
    }
    ...
    const appendAssistant = (delta: string) => {
      ...
      if (assistantContentBytes + deltaBytes > MAX_ASSISTANT_CONTENT_BYTES) {
        throw new RelayPlaygroundError('Assistant 响应超过 4 MiB，已停止读取', { kind: 'protocol_error' });
      }
    ...
    try {
      ...
      const relayError = isRelayError(reason)
        ? reason
        : new Error(reason instanceof Error ? reason.message : '调用失败') as RelayPlaygroundError;
    ...
    <div className="mx-auto max-w-[1600px] space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="outline" size="sm" render={<Link to="/usage" />}>
            查看使用记录
          </Button>
        </div>
      </div>
      ...
      {error ? (
        <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm dark:border-red-500/30 dark:bg-red-500/10">
          ...
          {error.kind === 'insufficient_quota' ? <Link className="font-bold text-red-700 underline dark:text-red-200" to="/recharge">前往充值</Link> : null}
        </div>
      ) : null}
        ...
        <Card className="rounded-2xl border-0 bg-white shadow-sm ring-1 ring-slate-200 dark:bg-card dark:ring-white/10">
          ...
          <CardContent className="space-y-5 p-5">
            <div className="space-y-2">
              <Label htmlFor="playground-address">Relay 地址</Label>
              <Input id="playground-address" readOnly value={address?.url || '正在读取…'} className="font-mono text-xs" />
              {address?.source === 'same-origin-fallback' ? <p className="text-xs text-amber-600 dark:text-amber-300">同源回退地址，请确认已配置反向代理。</p> : null}
            </div>
            <div className="space-y-2">
              <Label htmlFor="playground-key">API Key</Label>
              <div className="flex gap-2">
                <Input id="playground-key" type={showKey ? 'text' : 'password'} autoComplete="off" value={verifiedKey ? maskSecret(apiKey) : apiKey} onChange={(event) => changeApiKey(event.target.value)} placeholder="粘贴 sk-..." disabled={status === 'loading_models' || Boolean(activeRequest.current)} />
                <Button type="button" variant="outline" size="icon" onClick={() => setShowKey((value) => !value)} aria-label={showKey ? '隐藏 API Key' : '显示 API Key'} disabled={verifiedKey}>
            ...
            <details className="rounded-xl border border-slate-200 p-3 dark:border-white/10" open>
              ...
              <div className="mt-4 space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="playground-temperature" type="number" min="0" max="2" step="0.1" value={temperature} onChange={(event) => setTemperature(event.target.value)} placeholder="自动" disabled={Boolean(activeRequest.current)} />
                </div>
                <div className="space-y-2">
        ...
        <Card className="rounded-2xl border-0 bg-white shadow-sm ring-1 ring-slate-200 dark:bg-card dark:ring-white/10">
          ...
          <CardContent className="space-y-4 p-5">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <Metric label="总耗时" value={formatDuration(inspector.startedAt, inspector.completedAt)} />
